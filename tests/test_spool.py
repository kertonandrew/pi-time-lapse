...
class SpoolTests(unittest.TestCase):
    ...
    def test_capacity_check_observes_file_growth_after_previous_scan(self):
        image = self.spool.images / ".capture.part"
        image.write_bytes(b"a" * 10)
        self.spool.max_bytes = 20
        self.spool.check_capacity(10)
        image.write_bytes(b"a" * 15)
        with self.assertRaises(CapacityError):
            self.spool.check_capacity(10)

    def test_capacity_scan_rejects_symlinks_in_every_spool_directory(self):
        external = Path(self.directory.name) / "external.jpg"
        external.write_bytes(JPEG)
        for directory in (self.spool.images, self.spool.metadata, self.spool.receipts):
            with self.subTest(directory=directory.name):
                link = directory / "unexpected"
                link.symlink_to(external)
                try:
                    with self.assertRaises(SpoolError):
                        self.spool.check_capacity(0)
                finally:
                    link.unlink()
