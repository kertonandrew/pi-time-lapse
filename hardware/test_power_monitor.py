...
class SignalTests(Fixture):
    def test_signals_during_wait_flush_buffered_rows_without_deadlock(self):
        program = textwrap.dedent(
            ...
            lines, first_line = inspect.getsourcelines(threading.Event.wait)
            locked_line = first_line + next(
                index for index, line in enumerate(lines)
                if 'signaled = self._flag' in line
            )

            def run(buffer, interval, flush_seconds, stop):
                ...
                def trace(frame, event, argument):
                    if (
                        ...
                        and frame.f_lineno == locked_line
                        and frame.f_locals.get('self') is stop
                        and buffer.sample_index == 2
                    ):
                        signal.raise_signal(number)
                    return trace

                def wait(timeout):
                    if buffer.sample_index == 2:
                        timer = threading.Timer(
                            0.05, lambda: os.kill(os.getpid(), number)
                        )
                        timer.daemon = True
                        timer.start()
                ...
                try:
                    original_run(
                        buffer, interval=0.01, stop=stop, factory=object,
                        sample=sample, recover=lambda *args: None,
                    )
                finally:
                    sys.settrace(None)

            monitor.run_monitor = run
            raise SystemExit(monitor.main(['--database', sys.argv[1]]))
            """
        )
        for number in (signal.SIGTERM, signal.SIGINT):
            with self.subTest(signal=number):
                database = self.root / f"signal-{number}.sqlite3"
                result = subprocess.run(
                    [sys.executable, "-c", program, str(database), str(number)],
                ...
                with contextlib.closing(sqlite3.connect(database)) as connection:
                    rows = connection.execute(
                        "SELECT sample_index FROM samples ORDER BY sample_index"
                    ).fetchall()
                self.assertEqual(rows, [(0,), (1,)])

    def test_signal_during_sampling_finishes_and_commits_the_sample(self):
        buffer = monitor.SampleBuffer(self.database)
        ...
        with monitor.signal_stop() as stop:
            monitor.run_monitor(
                buffer, stop=stop, factory=Mock(), sample=sample, recover=Mock()
            )
        self.assertEqual(len(self.committed()), 1)

    def test_unrelated_signal_does_not_end_wait_early(self):
        previous = signal.signal(signal.SIGUSR1, lambda *args: None)
        try:
            with monitor.signal_stop() as stop:
                timer = threading.Timer(
                    0.02, lambda: os.kill(os.getpid(), signal.SIGUSR1)
                )
                timer.start()
                try:
                    started = time.monotonic()
                    self.assertFalse(stop.wait(0.1))
    ...
    def test_exception_restores_handlers_wakeup_fd_and_closes_pipe(self):
        ...
        try:
            with patch.object(monitor.os, "pipe", side_effect=open_pipe):
                with self.assertRaisesRegex(RuntimeError, "monitor failed"):
                    with monitor.signal_stop():
                        raise RuntimeError("monitor failed")
            self.assertEqual(signal.set_wakeup_fd(writer), writer)
            self.assertEqual(
                {number: signal.getsignal(number) for number in handlers}, handlers
            )
