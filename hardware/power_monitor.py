...
@contextmanager
def signal_stop():
    """Provide is_set and wait methods for SIGTERM and SIGINT shutdown."""
    stopping = False
    ...
    with ExitStack() as cleanup:
        ...
        previous_fd = signal.set_wakeup_fd(writer, warn_on_full_buffer=False)
        cleanup.callback(signal.set_wakeup_fd, previous_fd)
        ...
        def wait(timeout=None):
            ...
            while not stopping:
                delay = None if deadline is None else deadline - time.monotonic()
                if delay is not None and delay <= 0:
                    break
                readable, _, _ = select.select([reader], [], [], delay)
                if not readable:
                    break
                os.read(reader, 4096)
            return stopping

        yield SimpleNamespace(is_set=lambda: stopping, wait=wait)
